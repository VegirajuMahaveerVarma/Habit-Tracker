...
class _AppShellState extends State<AppShell> {
  ...
  Future<void> addHabit() async {
    ...
    setState(() => habits.add(r.makeHabit()));
    await saveHabits();
  }
  ...
  Future<void> editHabit(Habit h) async {
    ...
    if (r.delete) {
      final yes = await confirmDialog(context, 'Delete habit?', 'This removes the habit from your tracker.');
      if (yes) {
  ...
  Future<void> addTodo() async {
    ...
    setState(() => todos.add({
          ...
          'priority': r.priority,
          'category': r.category,
          'due': r.due == null ? '' : Habit.key(r.due!),
        }));
  ...
  Future<void> openTimer() async {
    await Navigator.push(context, MaterialPageRoute(builder: (_) => FocusTimer(
      storage: storage,
      date: date,
      initial: timers[Habit.key(date)] ?? 0,
  ...
  Widget build(BuildContext context) {
    ...
    final pages = [
      Dashboard(habits: habits, todos: todos, moods: moods, date: date, search: search, onSearch: (v) => setState(() => search = v), onToggle: toggleHabit, onAdd: addHabit, onEdit: editHabit, onMood: setMood, onTimer: openTimer),
      Daily(habits: habits, date: date, onDate: (v) => setState(() => date = v), onToggle: toggleHabit, onAdd: addHabit, onEdit: editHabit),
      Analytics(habits: habits, moods: moods, todos: todos),
      Todos(todos: todos, onAdd: addTodo, onEdit: editTodo, onToggle: toggleTodo),
      Profile(habits: habits, onPin: setPin, onTimer: openTimer),
    ];
    return Scaffold(
...
class Section extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Padding(padding: const EdgeInsets.fromLTRB(20, 18, 20, 10), child: Row(children: [Expanded(child: Text(title, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900))), if (onAdd != null) TextButton(onPressed: onAdd, child: const Text('＋ Add'))]));
}
...
class Dashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final visible = active.where((h) => q.isEmpty || h.name.toLowerCase().contains(q) || h.category.toLowerCase().contains(q) || h.tags.any((x) => x.toLowerCase().contains(q))).toList();
    final done = active.where((h) => h.isDone(date)).length;
    final p = active.isEmpty ? 0.0 : done / active.length;
    ...
    return SingleChildScrollView(padding: const EdgeInsets.only(bottom: 25), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: CardBox(child: Row(children: [SizedBox(width: 80, height: 80, child: Stack(fit: StackFit.expand, children: [CircularProgressIndicator(value: p, strokeWidth: 9, backgroundColor: AppColors.blueSoft, color: AppColors.blue), Center(child: Text('${(p * 100).round()}%', style: const TextStyle(fontWeight: FontWeight.w900)))])), const SizedBox(width: 18), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(done == active.length && active.isNotEmpty ? 'All done!' : 'Today’s progress', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900)), Text('$done of ${active.length} habits completed', style: const TextStyle(color: AppColors.muted)), const SizedBox(height: 9), LinearProgressIndicator(value: p, minHeight: 7, backgroundColor: AppColors.blueSoft)]))]))),
      Section(title: 'Quick stats'),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Row(children: [Expanded(child: CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.local_fire_department_outlined, color: AppColors.blue), const SizedBox(height: 6), const Text('Best streak', style: TextStyle(color: AppColors.muted)), Text('$best days', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18))]))), const SizedBox(width: 10), Expanded(child: CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Icon(Icons.list_alt_outlined, color: AppColors.blue), const SizedBox(height: 6), const Text('Open tasks', style: TextStyle(color: AppColors.muted)), Text('${todos.where((x) => x['done'] != true).length}', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18))])))])),
      Section(title: 'How are you feeling today?'),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: CardBox(child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: List.generate(5, (i) { final v = i + 1; return InkWell(onTap: () => onMood(v), child: Padding(padding: const EdgeInsets.all(8), child: Opacity(opacity: mood == v ? 1 : .55, child: Text(['😫', '😕', '😐', '🙂', '😄'][i], style: const TextStyle(fontSize: 25))))); })))),
      Section(title: q.isEmpty ? 'Today’s habits' : 'Search results', onAdd: onAdd),
      if (visible.isEmpty) const Padding(padding: EdgeInsets.all(30), child: Center(child: Text('No matching habits.', style: TextStyle(color: AppColors.muted)))) else ...visible.map((h) => Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 5), child: HabitTile(habit: h, date: date, onToggle: () => onToggle(h, date), onEdit: () => onEdit(h)))),
    ]));
  }
}
...
class Daily extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [Header(title: 'Daily', subtitle: DateFormat('MMMM yyyy').format(date)), SizedBox(height: 82, child: ListView.separated(padding: const EdgeInsets.symmetric(horizontal: 20), scrollDirection: Axis.horizontal, itemCount: days.length, separatorBuilder: (_, __) => const SizedBox(width: 8), itemBuilder: (_, i) { final d = days[i]; final selected = Habit.key(d) == Habit.key(date); final count = active.where((h) => h.isDone(d)).length; return InkWell(onTap: () => onDate(d), child: Container(width: 48, padding: const EdgeInsets.symmetric(vertical: 8), decoration: BoxDecoration(color: selected ? AppColors.blue : Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: selected ? AppColors.blue : AppColors.border)), child: Column(children: [Text(DateFormat('E').format(d).substring(0, 1), style: TextStyle(color: selected ? Colors.white : AppColors.muted)), Text('${d.day}', style: TextStyle(fontWeight: FontWeight.w900, color: selected ? Colors.white : AppColors.text)), Text('$count', style: TextStyle(color: selected ? Colors.white : AppColors.blue, fontSize: 11))]))); })), Section(title: '${DateFormat('EEE, d MMMM').format(date)} · $complete/${active.length} complete', onAdd: onAdd), Expanded(child: ListView(children: active.map((h) => Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 5), child: HabitTile(habit: h, date: date, onToggle: () => onToggle(h, date), onEdit: () => onEdit(h)))).toList()))]);
  }
}
...
class Analytics extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final avgMood = moods.isEmpty ? 0.0 : moods.values.reduce((a, b) => a + b) / moods.length;
    final days = List.generate(7, (i) => DateTime(now.year, now.month, now.day - 6 + i));
    return SingleChildScrollView(padding: const EdgeInsets.only(bottom: 25), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Header(title: 'Analytics', subtitle: 'Consistency, patterns and progress'), Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: CardBox(child: Row(children: [SizedBox(width: 78, height: 78, child: Stack(fit: StackFit.expand, children: [CircularProgressIndicator(value: overall, strokeWidth: 8, backgroundColor: AppColors.blueSoft, color: AppColors.blue), Center(child: Text('${(overall * 100).round()}%', style: const TextStyle(fontWeight: FontWeight.w900)))])), const SizedBox(width: 18), Expanded(child: Text('${active.length} active habits\n$total total completions\n$best best streak\n${todos.where((x) => x['done'] != true).length} open tasks\nAverage mood: ${avgMood == 0 ? '—' : '${avgMood.toStringAsFixed(1)}/5'}', style: const TextStyle(color: AppColors.muted, height: 1.5)))]))), Section(title: 'Last 7 days'), Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: CardBox(child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: days.map((d) { final n = active.where((h) => h.isDone(d)).length; final height = active.isEmpty ? 8.0 : (80.0 * n / active.length).clamp(8.0, 80.0).toDouble(); return Expanded(child: Column(children: [Text('$n', style: const TextStyle(fontSize: 11)), const SizedBox(height: 4), Container(height: height, margin: const EdgeInsets.symmetric(horizontal: 4), decoration: BoxDecoration(color: AppColors.blue, borderRadius: BorderRadius.circular(8))), const SizedBox(height: 4), Text(DateFormat('E').format(d).substring(0, 1), style: const TextStyle(color: AppColors.muted))])); }).toList()))), Section(title: 'Habit performance'), Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: CardBox(child: Column(children: active.map((h) => Padding(padding: const EdgeInsets.symmetric(vertical: 7), child: Row(children: [Expanded(child: Text(h.name, style: const TextStyle(fontWeight: FontWeight.w800))), Text('${(h.completionRate(now) * 100).round()}%'), const SizedBox(width: 8), SizedBox(width: 65, child: LinearProgressIndicator(value: h.completionRate(now).clamp(0.0, 1.0).toDouble()))])).toList()))), Section(title: 'Weekly review'), Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Best streak: $best days'), Text('Total completions: $total'), Text('Average mood: ${avgMood == 0 ? 'no entries' : '${avgMood.toStringAsFixed(1)}/5'}'), const SizedBox(height: 8), const Text('Review missed days and habit notes each week.', style: TextStyle(color: AppColors.muted))]))) ]));
  }
}
...
class Todos extends StatelessWidget {
  final List<Map<String, dynamic>> todos;
  final VoidCallback onAdd;
  final Future<void> Function(int) onEdit, onToggle;
...
class HabitEditResult {
  ...
  Habit makeHabit() => Habit(id: DateTime.now().microsecondsSinceEpoch.toString(), name: name, category: category, goal: goal, unit: unit, frequency: frequency, weekdays: weekdays, pinned: pinned, tags: tags, notes: notes);
}
...
class FocusTimer extends StatefulWidget {
  final StorageService storage; final DateTime date; final int initial; final ValueChanged<int> onSaved;
...
class _FocusTimerState extends State<FocusTimer> {
  late int seconds; Timer? timer; bool running = false;
  ...
  String fmt(int s) => '${(s ~/ 3600).toString().padLeft(2,'0')}:${((s % 3600) ~/ 60).toString().padLeft(2,'0')}:${(s % 60).toString().padLeft(2,'0')}';
...
class LockScreen extends StatelessWidget {
  final String pin; final VoidCallback onUnlock;
