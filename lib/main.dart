...
class _AppShellState extends State<AppShell> {
  ...
  Future<void> _toggle(Habit h, [DateTime? d]) async {
    final day = d ?? selectedDate;
    setState(() => h.setDone(day, !h.isDone(day)));
  ...
  Future<void> _reminderSettings() async {
  ...
  Widget build(BuildContext context) {
    final pages = [
      ...
      DailyPage(habits: habits, date: selectedDate, onDateChanged: (d) => setState(() => selectedDate = d), onToggle: _toggle, onAdd: _addHabit, onEdit: _editHabit),
      AnalyticsPage(habits: habits),
      TodoPage(todos: todos, onAdd: _addTodo, onEdit: _editTodo, onChanged: () => storage.saveTodos(todos)),
      ProfilePage(habits: habits, reminders: reminders, onReminders: _reminderSettings),
    ];
    return Scaffold(
...
class PageHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
        ...
        child: Row(
          children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 29, fontWeight: FontWeight.w900, color: AppColors.text)), if (subtitle != null) Text(subtitle!, style: const TextStyle(color: AppColors.muted, fontSize: 14))])),
...
class MiniStat extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => SurfaceCard(padding: const EdgeInsets.all(14), child: Row(children: [Icon(icon, color: AppColors.blue, size: 20), const SizedBox(width: 9), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: const TextStyle(color: AppColors.muted, fontSize: 11)), const SizedBox(height: 2), Text(value, style: const TextStyle(fontWeight: FontWeight.w900))]))]));
}
...
class DashboardPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final md = habits.fold<int>(0, (s, h) => s + h.completedInMonth(date));
    final g = habits.fold<int>(0, (s, h) => s + h.goal);
    return SingleChildScrollView(padding: const EdgeInsets.only(bottom: 20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Row(children: [Expanded(child: MiniStat(icon: Icons.calendar_month_outlined, label: 'This month', value: '$md days')), const SizedBox(width: 10), Expanded(child: MiniStat(icon: Icons.flag_outlined, label: 'Goal progress', value: g == 0 ? '0%' : '${(md / g * 100).round()}%'))])),
...
class HabitTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final m = habit.completedInMonth(date);
    final r = habit.goal == 0 ? 0.0 : (m / habit.goal).clamp(0.0, 1.0);
    return Padding(
                      ...
                      Row(
                        children: [
                          ...
                          Container(
                            ...
                            child: Text(habit.category, style: const TextStyle(fontSize: 10, color: AppColors.blue, fontWeight: FontWeight.w700)),
                          ),
                        ],
                      ),
                      const SizedBox(height: 9),
                      LinearProgressIndicator(value: r, minHeight: 5, backgroundColor: AppColors.bg),
                      const SizedBox(height: 5),
                      Text('$m/${habit.goal} this month', style: const TextStyle(fontSize: 11, color: AppColors.muted)),
                    ],
                  ),
                ),
                const SizedBox(width: 8),
                Text('$m/${habit.goal}', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w900, color: AppColors.muted)),
              ],
            ),
          ),
        ),
...
class DailyPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      SizedBox(height: 86, child: ListView.builder(scrollDirection: Axis.horizontal, padding: const EdgeInsets.symmetric(horizontal: 14), itemCount: days, itemBuilder: (_, i) {
        final d = DateTime(date.year, date.month, i + 1);
        final s = DateUtils.isSameDay(d, date);
        return GestureDetector(onTap: () => onDateChanged(d), child: Container(width: 48, margin: const EdgeInsets.symmetric(horizontal: 3, vertical: 6), decoration: BoxDecoration(color: s ? AppColors.blue : Colors.white, borderRadius: BorderRadius.circular(15), border: Border.all(color: s ? AppColors.blue : AppColors.border)), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Text(DateFormat('EEE').format(d).substring(0, 2), style: TextStyle(fontSize: 11, color: s ? Colors.white70 : AppColors.muted)), const SizedBox(height: 5), Text('${d.day}', style: TextStyle(fontWeight: FontWeight.w900, color: s ? Colors.white : AppColors.text))])));
      })),
...
class AnalyticsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ds = List.generate(7, (i) => DateTime(now.year, now.month, now.day - 6 + i));
    final vals = ds.map((d) => habits.where((h) => h.isDone(d)).length).toList();
    ...
    return SingleChildScrollView(padding: const EdgeInsets.only(bottom: 24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: SurfaceCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Last 7 days', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)), const SizedBox(height: 20), SizedBox(height: 150, child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: List.generate(7, (i) => Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [Text('${vals[i]}', style: const TextStyle(fontSize: 11, color: AppColors.muted)), const SizedBox(height: 5), Container(height: 16 + 100 * vals[i] / max, decoration: BoxDecoration(color: i == 6 ? AppColors.blue : AppColors.blueSoft, borderRadius: const BorderRadius.vertical(top: Radius.circular(9)))), const SizedBox(height: 7), Text(DateFormat('EEE').format(ds[i]).substring(0, 2), style: const TextStyle(fontSize: 10, color: AppColors.muted))]))))]))),
      const SizedBox(height: 18),
      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: SurfaceCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Habit progress', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900)), const SizedBox(height: 12), ...habits.map((h) { final v = h.completedInMonth(now); final r = h.goal == 0 ? 0.0 : (v / h.goal).clamp(0.0, 1.0); return Padding(padding: const EdgeInsets.only(bottom: 13), child: Column(children: [Row(children: [Expanded(child: Text(h.displayName, style: const TextStyle(fontWeight: FontWeight.w700))), Text('$v/${h.goal}', style: const TextStyle(fontSize: 12, color: AppColors.muted, fontWeight: FontWeight.w800))]), const SizedBox(height: 7), LinearProgressIndicator(value: r, minHeight: 6, backgroundColor: AppColors.bg)])); })]))),
    ]));
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      Expanded(child: widget.todos.isEmpty ? const EmptyState(icon: Icons.task_alt, title: 'Nothing here yet', message: 'Add tasks and check them off as you go.') : ListView.builder(padding: const EdgeInsets.only(bottom: 20), itemCount: widget.todos.length, itemBuilder: (_, i) {
        ...
        final d = t['done'] == true;
        return Padding(padding: const EdgeInsets.fromLTRB(20, 0, 20, 10), child: Dismissible(key: ValueKey('${t['title']}_$i'), direction: DismissDirection.endToStart, background: Container(alignment: Alignment.centerRight, padding: const EdgeInsets.only(right: 20), decoration: BoxDecoration(color: Colors.red.shade50, borderRadius: BorderRadius.circular(16)), child: const Icon(Icons.delete_outline, color: Colors.red)), onDismissed: (_) async { setState(() => widget.todos.removeAt(i)); await widget.onChanged(); }, child: Material(color: Colors.white, borderRadius: BorderRadius.circular(16), child: ListTile(onLongPress: () => widget.onEdit(i), contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16), side: const BorderSide(color: AppColors.border)), leading: IconButton(onPressed: () async { setState(() => t['done'] = !d); await widget.onChanged(); }, icon: Icon(d ? Icons.check_circle : Icons.radio_button_unchecked, color: d ? AppColors.green : AppColors.muted)), title: Text('${t['title']}', style: TextStyle(fontWeight: FontWeight.w700, decoration: d ? TextDecoration.lineThrough : null, color: d ? AppColors.muted : AppColors.text))))));
      }))
    ]);
  }
}
...
class _TodoEditorDialogState extends State<TodoEditorDialog> {
  ...
  @override void initState() { super.initState(); c = TextEditingController(text: widget.initialTitle ?? ''); }
  @override void dispose() { c.dispose(); super.dispose(); }
  void save() { final t = c.text.trim(); if (t.isEmpty) return; Navigator.pop(context, TodoEditResult(title: t)); }
  @override
  Widget build(BuildContext context) { final e = widget.initialTitle != null; return AlertDialog(title: Text(e ? 'Edit task' : 'Add task', style: const TextStyle(fontWeight: FontWeight.w800)), content: TextField(controller: c, autofocus: true, textInputAction: TextInputAction.done, onSubmitted: (_) => save(), decoration: const InputDecoration(hintText: 'What needs to be done?')), actions: [if (e) TextButton(onPressed: () => Navigator.pop(context, const TodoEditResult(delete: true)), child: const Text('Delete')), TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')), FilledButton(onPressed: save, child: Text(e ? 'Save' : 'Add'))]); }
}
...
class _HabitEditorSheetState extends State<HabitEditorSheet> {
  ...
  late double goal;
  static const cats = ['Daily', 'Mind & Body', 'Productivity', 'Goals'];
  @override void initState() { super.initState(); c = TextEditingController(text: widget.habit?.name ?? ''); category = cats.contains(widget.habit?.category) ? widget.habit!.category : 'Daily'; goal = (widget.habit?.goal ?? 30).clamp(1, 31).toDouble(); }
  @override void dispose() { c.dispose(); super.dispose(); }
  void save() { final n = c.text.trim(); if (n.isEmpty) return; Navigator.pop(context, HabitEditResult(name: n, category: category, goal: goal.round())); }
  @override Widget build(BuildContext context) => SafeArea(child: Padding(padding: EdgeInsets.fromLTRB(20, 12, 20, MediaQuery.viewInsetsOf(context).bottom + 20), child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Center(child: Container(width: 42, height: 4, decoration: BoxDecoration(color: AppColors.border, borderRadius: BorderRadius.circular(10)))), const SizedBox(height: 16), Row(children: [Expanded(child: Text(widget.habit != null ? 'Edit habit' : 'Create a habit', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900))), IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close))]), const SizedBox(height: 8), TextField(controller: c, autofocus: true, onSubmitted: (_) => save(), decoration: const InputDecoration(labelText: 'Habit name', prefixIcon: Icon(Icons.flag_outlined))), const SizedBox(height: 12), DropdownButtonFormField<String>(initialValue: category, decoration: const InputDecoration(labelText: 'Category', prefixIcon: Icon(Icons.category_outlined)), items: cats.map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(), onChanged: (v) { if (v != null) setState(() => category = v); }), const SizedBox(height: 14), Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: AppColors.bg, borderRadius: BorderRadius.circular(16)), child: Column(children: [Row(children: [const Expanded(child: Text('Monthly goal', style: TextStyle(fontWeight: FontWeight.w800))), Text('${goal.round()} days', style: const TextStyle(color: AppColors.blue, fontWeight: FontWeight.w900))]), Slider(value: goal, min: 1, max: 31, divisions: 30, onChanged: (v) => setState(() => goal = v))])), const SizedBox(height: 14), Row(children: [if (widget.habit != null) ...[Expanded(child: OutlinedButton.icon(onPressed: () => Navigator.pop(context, const HabitEditResult(delete: true)), icon: const Icon(Icons.delete_outline), label: const Text('Delete'))), const SizedBox(width: 12)], Expanded(child: FilledButton.icon(onPressed: save, icon: const Icon(Icons.check), label: Text(widget.habit != null ? 'Save changes' : 'Create habit')))])]))));
}
...
class _ReminderSettingsSheetState extends State<ReminderSettingsSheet> {
  ...
  @override Widget build(BuildContext context) => SafeArea(child: Padding(padding: EdgeInsets.fromLTRB(20, 12, 20, MediaQuery.viewInsetsOf(context).bottom + 20), child: Column(mainAxisSize: MainAxisSize.min, children: [Center(child: Container(width: 42, height: 4, decoration: BoxDecoration(color: AppColors.border, borderRadius: BorderRadius.circular(10)))), const SizedBox(height: 16), Row(children: [const Expanded(child: Text('Habit reminders', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900))), IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close))]), SwitchListTile(contentPadding: EdgeInsets.zero, title: const Text('Daily reminders', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: const Text('Notify me for active habits every day.'), value: enabled, onChanged: (v) => setState(() => enabled = v)), ListTile(enabled: enabled, onTap: enabled ? () async { final p = await showTimePicker(context: context, initialTime: time); if (p != null) setState(() => time = p); } : null, leading: const Icon(Icons.alarm, color: AppColors.blue), title: const Text('Reminder time', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: Text(time.format(context)), trailing: const Icon(Icons.chevron_right)), const SizedBox(height: 8), Row(children: [Expanded(child: OutlinedButton.icon(onPressed: () => NotificationService.instance.showTestNotification(), icon: const Icon(Icons.notifications_active_outlined), label: const Text('Test notification'))), const SizedBox(width: 10), Expanded(child: FilledButton.icon(onPressed: () => Navigator.pop(context, ReminderSettings(enabled: enabled, hour: time.hour, minute: time.minute)), icon: const Icon(Icons.check), label: const Text('Save')))]), const SizedBox(height: 8), const Text('Runs locally on this device. No account or paid service is required.', textAlign: TextAlign.center, style: TextStyle(fontSize: 12, color: AppColors.muted))])));
}
...
class ProfilePage extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) { final done = habits.fold<int>(0, (s, h) => s + h.completedInMonth(DateTime.now())); return SingleChildScrollView(padding: const EdgeInsets.only(bottom: 24), child: Column(children: [const SizedBox(height: 20), Container(width: 82, height: 82, decoration: const BoxDecoration(color: AppColors.navy, shape: BoxShape.circle), child: const Icon(Icons.person, color: Colors.white, size: 40)), const SizedBox(height: 12), const Text('Habit Builder', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900)), const SizedBox(height: 4), const Text('Build better days, one habit at a time.', style: TextStyle(color: AppColors.muted)), const SizedBox(height: 24), Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Row(children: [Expanded(child: StatCard(icon: Icons.check_circle_outline, title: 'Habits', value: '${habits.length}')), const SizedBox(width: 10), Expanded(child: StatCard(icon: Icons.done_all, title: 'This month', value: '$done'))])), const SizedBox(height: 18), Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: SurfaceCard(child: Column(children: [ListTile(contentPadding: EdgeInsets.zero, onTap: onReminders, leading: const Icon(Icons.notifications_active_outlined, color: AppColors.blue), title: const Text('Habit reminders', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: Text(reminders.enabled ? 'Daily at ${TimeOfDay(hour: reminders.hour, minute: reminders.minute).format(context)}' : 'Off'), trailing: const Icon(Icons.chevron_right)), const Divider(height: 1), const ListTile(contentPadding: EdgeInsets.zero, leading: Icon(Icons.lock_outline, color: AppColors.green), title: Text('Private & offline', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('Your habit data stays on this device.')), const Divider(height: 1), const ListTile(contentPadding: EdgeInsets.zero, leading: Icon(Icons.tips_and_updates_outlined), title: Text('Tip', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: Text('Long-press habits or To-Do tasks to edit them.'))])))])); }
}
