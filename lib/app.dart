...
class _HomeShellState extends State<HomeShell> {
  ...
  Future<void> load() async {
    ...
    timers = await store.loadTimerSeconds();
  ...
  Future<void> editTodo(int index) async {
    final form = await showDialog<TodoForm>(context: context, builder: (_) => TodoEditor(existing: todos[index]));
    if (form == null) return;
    todos[index]['title'] = form.title.trim();
    todos[index]['category'] = form.category;
    todos[index]['priority'] = form.priority;
    setState(() {});
    await store.saveTodos(todos);
  }
  ...
  Future<void> openTimer() async {
    ...
    final value = await Navigator.push<int>(context, MaterialPageRoute(builder: (_) => FocusTimer(initial: timers[key] ?? 0)));
  ...
  Future<void> setPin() async {
    final value = await showDialog<String>(context: context, builder: (_) => PinDialog());
    if (value == null) return;
    await store.savePin(value);
    setState(() { pin = value.isEmpty ? null : value; locked = false; });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final pages = <Widget>[
      ...
      Profile(habits: habits, hasPin: pin != null, onTimer: openTimer, onPin: setPin),
    ];
    return Scaffold(
...
class Dashboard extends StatefulWidget {
  ...
  const Dashboard({super.key, required this.habits, required this.date, required this.moods, required this.onToggle, required this.onEdit, required this.onOpen, required this.onAdd, required this.onMood, required this.onTimer});
...
class _DashboardState extends State<Dashboard> {
  ...
  Widget build(BuildContext context) {
    ...
    final visible = active.where((h) {
      ...
      final textMatch = q.isEmpty || h.name.toLowerCase().contains(q) || h.tags.any((t) => t.toLowerCase().contains(q));
      final filterMatch = filter == 'All' || (filter == 'Pinned' && h.pinned) || (filter == 'Completed' && h.isDone(widget.date)) || (filter == 'Missed' && h.isScheduled(widget.date) && !h.isDone(widget.date));
...
class _StatsState extends State<Stats> {
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(padding: const EdgeInsets.only(bottom: 30), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: CardBox(child: WeeklyReview(habits: active))),
    ]));
  }
}
...
class NeedsAttention extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final now = DateTime.now();
    final items = habits.where((h) => h.totalCompletions() > 0 && h.completionRate(now) < 0.6).take(4).toList();
    if (items.isEmpty) return const ListTile(contentPadding: EdgeInsets.zero, leading: Icon(Icons.check_circle, color: AppColors.green), title: Text('Everything is on track'));
    final children = <Widget>[];
    for (final h in items) {
      children.add(ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.trending_down, color: AppColors.red), title: Text(h.name), subtitle: Text('${(h.completionRate(now) * 100).round()}% this month')));
    }
...
class WeeklyReview extends StatelessWidget {
  final List<Habit> habits;
  const WeeklyReview({super.key, required this.habits});
  @override
  Widget build(BuildContext context) {
    var total = 0;
    var done = 0;
...
String priorityName(num? value) {
  final n = value?.toInt() ?? 1;
  if (n == 3) return 'High';
...
class Profile extends StatelessWidget {
  ...
  const Profile({super.key, required this.habits, required this.hasPin, required this.onTimer, required this.onPin});
  @override
  Widget build(BuildContext context) {
...
class Achievements extends StatelessWidget {
  final int total;
  final int best;
  const Achievements({super.key, required this.total, required this.best});
  @override
  Widget build(BuildContext context) {
    final data = <List<Object>>[
      <Object>['🌱', 'First step', total >= 1],
      <Object>['🔥', '7 day streak', best >= 7],
...
class HabitDetail extends StatefulWidget {
  ...
  const HabitDetail({super.key, required this.habit, required this.onSave});
...
class FocusTimer extends StatefulWidget {
  final int initial;
  const FocusTimer({super.key, this.initial = 0});
