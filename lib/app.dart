...
class _HomeShellState extends State<HomeShell> {
  ...
  Future<void> addHabit() async {
    final r = await showModalBottomSheet<HabitEdit>(context: context, isScrollControlled: true, builder: (_) => const HabitEditor());
  ...
  Future<void> editHabit(Habit h) async {
    final r = await showModalBottomSheet<HabitEdit>(context: context, isScrollControlled: true, builder: (_) => HabitEditor(habit: h));
    ...
    if (r.delete) {
      ...
      h.active = r.active; h.tags = r.tags; h.notes = r.notes;
      setState(() {});
    }
    await saveHabits();
  }
  ...
  Future<void> addTodo() async {
    ...
    final item = {'id': DateTime.now().microsecondsSinceEpoch.toString(), 'title': r.title.trim(), 'done': false, 'priority': r.priority, 'category': r.category, 'due': r.due == null ? '' : Habit.key(r.due!)};
    setState(() => todos.add(item));
    await store.saveTodos(todos);
  }

  Future<void> editTodo(int index) async {
    final r = await showDialog<TodoEdit>(context: context, builder: (_) => TodoEditor(existing: todos[index]));
    if (!mounted || r == null) return;
    setState(() {
      todos[index]['title'] = r.title.trim();
      todos[index]['priority'] = r.priority;
      todos[index]['category'] = r.category;
      todos[index]['due'] = r.due == null ? '' : Habit.key(r.due!);
    });
    await store.saveTodos(todos);
  }
  ...
  Future<void> setPin() async {
    final r = await showDialog<String>(context: context, builder: (_) => PinDialog(current: pin));
    if (r == null) return;
    await store.savePin(r);
    if (mounted) setState(() => pin = r.isEmpty ? null : r);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final pages = <Widget>[
      Dashboard(habits: habits, moods: moods, date: selected, onAdd: addHabit, onTimer: openTimer, onToggle: toggleHabit, onOpen: openHabit, onEdit: editHabit, onMood: (v) async { setState(() => moods[Habit.key(selected)] = v); await store.saveMoods(moods); }),
      Daily(habits: habits, date: selected, onDate: (d) => setState(() => selected = d), onAdd: addHabit, onToggle: toggleHabit, onOpen: openHabit, onEdit: editHabit),
...
class PageHeader extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: AppColors.text)), Text(subtitle, style: const TextStyle(color: AppColors.muted))])),
...
class SectionTitle extends StatelessWidget {
  final String title, label; final VoidCallback? onAdd;
...
class Dashboard extends StatefulWidget {
  final List<Habit> habits; final Map<String,int> moods; final DateTime date;
...
class _DashboardState extends State<Dashboard> {
  ...
  @override Widget build(BuildContext context) {
    ...
    final visible = active.where((h) {
      ...
      final match = q.isEmpty || h.name.toLowerCase().contains(q) || h.category.toLowerCase().contains(q) || h.tags.any((t) => t.toLowerCase().contains(q));
    ...
    final streak = active.fold<int>(0, (m,h) => h.currentStreak(widget.date) > m ? h.currentStreak(widget.date) : m);
...
class _StatsState extends State<Stats>{String range='30D';int year=DateTime.now().year;String selectedHabit='All habits';
  int daysForRange(){if(range=='7D')return 7;if(range=='90D')return 90;if(range=='1Y')return 365;return 30;}
  double rateFor(DateTime start,DateTime end){var done=0,total=0;for(final h in widget.habits.where((x)=>x.active)){for(var d=start;!d.isAfter(end);d=d.add(const Duration(days:1))){if(h.isScheduled(d)){total++;if(h.isDone(d))done++;}}}return total==0?0:done/total;}
  int completionsFor(DateTime start,DateTime end){var n=0;for(final h in widget.habits.where((x)=>x.active)){for(var d=start;!d.isAfter(end);d=d.add(const Duration(days:1))){if(h.isDone(d))n++;}}return n;}
  @override Widget build(BuildContext context){final now=DateTime.now();final start=now.subtract(Duration(days:daysForRange()-1));final rate=rateFor(start,now);final total=completionsFor(start,now);final current=widget.habits.fold<int>(0,(m,h)=>h.currentStreak()>m?h.currentStreak():m);final best=widget.habits.fold<int>(0,(m,h)=>h.bestStreak()>m?h.bestStreak():m);return SingleChildScrollView(padding:const EdgeInsets.only(bottom:30),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[PageHeader(title:'Analytics',subtitle:'Your habit history and patterns'),Padding(padding:const EdgeInsets.symmetric(horizontal:20),child:Wrap(spacing:7,children:['7D','30D','90D','1Y'].map((x)=>ChoiceChip(label:Text(x),selected:range==x,onSelected:(_)=>setState(()=>range=x))).toList())),Padding(padding:const EdgeInsets.fromLTRB(20,14,20,0),child:GridView.count(crossAxisCount:2,shrinkWrap:true,physics:const NeverScrollableScrollPhysics(),crossAxisSpacing:10,mainAxisSpacing:10,childAspectRatio:1.7,children:[MiniStat(value:'${(rate*100).round()}%',label:'Completion rate',icon:Icons.percent),MiniStat(value:'$current',label:'Current streak',icon:Icons.local_fire_department),MiniStat(value:'$best',label:'Best streak',icon:Icons.emoji_events_outlined),MiniStat(value:'$total',label:'Completions',icon:Icons.check_circle_outline)])),SectionTitle(title:'Completion trend'),Padding(padding:const EdgeInsets.symmetric(horizontal:20),child:CardBox(child:TrendChart(habits:widget.habits,days:daysForRange()))),SectionTitle(title:'Contribution activity'),Padding(padding:const EdgeInsets.symmetric(horizontal:20),child:CardBox(padding:const EdgeInsets.all(14),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Row(children:[const Text('Activity heatmap',style:TextStyle(fontWeight:FontWeight.w900)),const Spacer(),DropdownButton<String>(value:selectedHabit,items:['All habits',...widget.habits.map((h)=>h.name)].map((x)=>DropdownMenuItem(value:x,child:Text(x,maxLines:1,overflow:TextOverflow.ellipsis))).toList(),onChanged:(v){if(v!=null)setState(()=>selectedHabit=v);})]),const SizedBox(height:8),SizedBox(height:132,child:ActivityHeatmap(habits:widget.habits,year:year,habitName:selectedHabit)),const SizedBox(height:8),Row(children:[const Text('Less',style:TextStyle(fontSize:11,color:AppColors.muted)),...List.generate(5,(i)=>Container(width:14,height:14,margin:const EdgeInsets.only(left:5),decoration:BoxDecoration(color:heatColor(i),borderRadius:BorderRadius.circular(3)))),const Text('  More',style:TextStyle(fontSize:11,color:AppColors.muted))])])),SectionTitle(title:'Day-of-week consistency'),Padding(padding:const EdgeInsets.symmetric(horizontal:20),child:CardBox(child:WeekdayAnalysis(habits:widget.habits))),SectionTitle(title:'Habit performance'),Padding(padding:const EdgeInsets.symmetric(horizontal:20),child:CardBox(child:Column(children:widget.habits.where((h)=>h.active).map((h){final r=h.completionRate(now);return ListTile(contentPadding:EdgeInsets.zero,onTap:()=>widget.onOpen(h),title:Text(h.name,style:const TextStyle(fontWeight:FontWeight.w800)),subtitle:LinearProgressIndicator(value:r.clamp(0.0,1.0).toDouble(),backgroundColor:AppColors.soft),trailing:Text('${(r*100).round()}%'));}).toList()))),SectionTitle(title:'Needs attention'),Padding(padding:const EdgeInsets.symmetric(horizontal:20),child:CardBox(child:NeedsAttention(habits:widget.habits))),SectionTitle(title:'Weekly review'),Padding(padding:const EdgeInsets.symmetric(horizontal:20),child:CardBox(child:WeeklyReview(habits:widget.habits,moods:widget.moods))),Padding(padding:const EdgeInsets.symmetric(horizontal:20,vertical:8),child:Row(children:[IconButton(onPressed:year>2020?()=>setState(()=>year--):null,icon:const Icon(Icons.chevron_left)),Expanded(child:Center(child:Text('$year heatmap',style:const TextStyle(fontWeight:FontWeight.w800)))),IconButton(onPressed:()=>setState(()=>year++),icon:const Icon(Icons.chevron_right))]))]));}
}
...
class _PinDialogState extends State<PinDialog>{final c=TextEditingController();@override void dispose(){c.dispose();super.dispose();}@override Widget build(BuildContext context)=>AlertDialog(title:Text(widget.current==null?'Set PIN':'Change PIN'),content:TextField(controller:c,keyboardType:TextInputType.number,obscureText:true,maxLength:4,decoration:const InputDecoration(hintText:'4 digits')),actions:[if(widget.current!=null)TextButton(onPressed:()=>Navigator.pop(context,''),child:const Text('Remove')),FilledButton(onPressed:(){if(c.text.length==4)Navigator.pop(context,c.text);},child:const Text('Save'))]);}
